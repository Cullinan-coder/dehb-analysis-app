...
import { BubbleTapResult, BubblesDifficulty } from '../../games/bubbles/types';
import { getDifficultyForAge } from '../../games/bubbles/logic';
import {
  createSession,
...
export default function BubblesScreen() {
  ...
  const [difficulty, setDifficulty] = useState<BubblesDifficulty>(() =>
  ...
  async function handleStartSession() {
    ...
    if (childId) {
      ...
      if (dbSession) {
        ...
        console.log('[Bubbles Session] Başlatıldı:', dbSession.id);
      }
    }
  }

  async function handleRoundComplete(result: BubbleTapResult, roundFinished: boolean) {
    recordTouch();
    if (result.isTarget) {
    ...
    if (dbSessionId) {
      await saveBehavioralEvent(dbSessionId, 'bubble_tap', {
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.startContainer}>
          ...
          <Text style={styles.title}>Harf Baloncukları</Text>
          <Text style={styles.subtitle}>Doğru baloncuğa dokunup patlat!</Text>
  ...
  if (showCompletionScreen) {
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.startContainer}>
          <Text style={styles.startEmoji}>🎉</Text>
          <Text style={styles.title}>Süpersin!</Text>
          <Text style={styles.subtitle}>
            {correctCount} baloncuk patlattın!
          </Text>
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.gameContainer}>
        <View style={styles.topBar}>
          ...
          <Stat label="Görev" value={`${completedTasks}/${BUBBLES_TOTAL_ROUNDS}`} />
        ...
        <View style={styles.gameArea}>
          <BubblesScene
            ...
            totalRounds={BUBBLES_TOTAL_ROUNDS}
...
const styles = StyleSheet.create({
  ...
  startButton: { backgroundColor: '#06b6d4', paddingHorizontal: 48, paddingVertical: 20, borderRadius: 30 },
